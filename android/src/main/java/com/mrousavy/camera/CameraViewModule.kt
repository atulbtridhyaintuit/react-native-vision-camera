...
fun onFaceDetected(temp: String) {
    Log.i(CameraView.TAG, "face detected successful")
    temporary = temp
}
...
class CameraViewModule(reactContext: ReactApplicationContext) :
    ...
    @Suppress("unused")
    @ReactMethod
    fun tempFaceDetected(viewTag: Int, mySuccessCallback: Callback) {
            findCameraView(viewTag).onFaceDetectedRN(mySuccessCallback)
    }
    ...
    @Suppress("unused")
    @ReactMethod
    fun getImageForFaceId(viewTag: Int, point: ReadableMap, promise: Promise) {
        Log.d("first Step", point.toString())
        coroutineScope.launch {
            withPromise(promise) {
                val view = findCameraView(viewTag)
                view.getImageForFace(point, promise)
                return@withPromise null
            }
        }
    }
        ...
        coroutineScope.launch {
            withPromise(promise) {
                ...
                val extensionsManager =
                    ExtensionsManager.getInstanceAsync(reactApplicationContext, cameraProvider)
                        .await()
                ProcessCameraProvider.getInstance(reactApplicationContext).await()
                ...
                manager.cameraIdList.forEach loop@{ id ->
                    ...
                    val supportsHdr = extensionsManager.isExtensionAvailable(
                        cameraSelector,
                        ExtensionMode.HDR
                    )
                    val supportsLowLightBoost = extensionsManager.isExtensionAvailable(
                        cameraSelector,
                        ExtensionMode.NIGHT
                    )
                    ...
                    map.putBoolean(
                        "supportsFocus",
                        true
                    ) // I believe every device here supports focussing
                    if (zoomRange != null) {
                    ...
                    cameraConfig.outputFormats.forEach { formatId ->
                        ...
                        cameraConfig.getOutputSizes(formatId).forEach { size ->
                            val isHighestPhotoQualitySupported =
                                areUltimatelyEqual(size, maxImageOutputSize)
                            ...
                            format.putDouble(
                                "maxZoom",
                                (zoomRange?.upper ?: maxScalerZoom).toDouble()
                            )
